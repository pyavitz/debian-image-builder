...
if [[ "$HOST_ARCH" =~ ^(aarch64|x86_64)$ ]]; then
	if [[ "$HOST_ARCH" == "x86_64" ]]; then sudo apt update; sudo apt upgrade -y; make ccompile; fi
	if [[ "$HOST_ARCH" == "aarch64" ]]; then sudo apt update; sudo apt upgrade -y; make ncompile; fi
else
